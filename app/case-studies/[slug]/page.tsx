...
const display = 'font-[family-name:var(--font-nunito)] font-extrabold tracking-[-0.01em]';
const mono = 'font-mono-label text-[0.66rem] font-semibold uppercase tracking-[0.18em]';
...
export default async function CaseStudyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden bg-[var(--color-page)] text-[var(--color-text)]">
      <div className="halo -left-[10%] -top-[6%] h-[34rem] w-[34rem]" style={{ background: 'var(--halo-a)' }} />
      ...
      <div className={`${shell} relative z-10 pb-20 pt-10`}>
        ...
        <section className="mt-10 grid gap-10 lg:grid-cols-[1fr_0.82fr] lg:items-start">
          <div className="max-w-2xl">
            ...
            <p className={`${mono} mt-3 text-[var(--color-soft)]`}>{project.category}</p>
            <h1 className={`${display} mt-4 text-[clamp(2.4rem,5vw,4.4rem)] leading-[0.95]`}>{project.title}</h1>
            <p className="mt-6 max-w-xl text-[1rem] leading-8 text-[var(--color-muted)]">{project.summary}</p>
            <div className="mt-7 flex flex-wrap gap-1.5">
              {project.highlights.map((item) => (
                <span key={item} className="rounded-md border border-[var(--border-subtle)] bg-[var(--color-raise)] px-2.5 py-1 font-mono-label text-[0.64rem] font-semibold text-[var(--color-muted)]">
                  {item}
                </span>
              ))}
            </div>
          ...
          <aside className="rounded-2xl border border-[var(--border-card)] bg-[var(--color-raise)] p-5">
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                <span className="grid size-12 place-items-center rounded-xl border border-[var(--border-card)] bg-[var(--color-page)]">
                  {project.logoUrl ? (
                    <Image src={project.logoUrl} alt="" width={32} height={32} className="size-8 object-contain" aria-hidden="true" />
                  ) : (
                    <project.icon size={20} className="text-[var(--color-accent)]" aria-hidden="true" />
                  )}
                </span>
                <div>
            ...
            {project.heroImageUrl ? (
              <div className="mt-5 overflow-hidden rounded-xl border border-[var(--border-card)]">
                <Image src={project.heroImageUrl} alt={`${project.title} screenshot`} width={1200} height={760} className="h-auto w-full object-cover" loading="lazy" />
              </div>
            ) : null}
            ...
            <div className="mt-5 grid grid-cols-2 gap-2.5">
              {[
                ...
                <div key={label} className="rounded-xl border border-[var(--border-card)] bg-[var(--color-page)] p-3">
        ...
        <section className="mt-10 grid gap-3 md:grid-cols-3">
          {[
            ...
            <article key={label} className="card-hover rounded-2xl border border-[var(--border-card)] bg-[var(--color-raise)] p-6">
              <span className="font-mono-label text-[0.7rem] font-semibold text-[var(--color-soft)]">0{i + 1}</span>
        ...
        <section className="mt-3 grid gap-3 lg:grid-cols-[1.05fr_0.95fr]">
          <article className="rounded-2xl border border-[var(--border-card)] bg-[var(--color-raise)] p-6">
            <p className={`${mono} text-[var(--color-accent)]`}>Responsibilities</p>
            <ul className="mt-4 grid gap-3 text-[0.92rem] leading-7 text-[var(--color-muted)]">
              {project.responsibilities.map((item) => (
                <li className="flex gap-3" key={item}>
                  <CheckCircle2 className="mt-0.5 shrink-0 text-[var(--color-accent-2)]" size={17} aria-hidden="true" />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
          ...
          <article className="rounded-2xl border border-[var(--border-card)] bg-[var(--color-raise)] p-6">
            ...
            <div className="mt-4 flex flex-wrap gap-1.5">
              {project.stack.map((item) => (
                <span key={item} className="rounded-md border border-[var(--border-subtle)] bg-[var(--color-page)] px-2.5 py-1 font-mono-label text-[0.64rem] font-semibold text-[var(--color-muted)]">
                  {item}
                </span>
              ))}
            </div>
            <div className="mt-5 rounded-xl border border-[var(--border-card)] bg-[var(--color-page)] p-4">
              ...
              <p className="mt-2 text-[0.9rem] leading-7 text-[var(--color-muted)]">{project.businessValue}</p>
            </div>
          </article>
        </section>
        ...
        <section className="mt-10 flex flex-col items-start justify-between gap-5 rounded-2xl border border-[var(--border-card)] bg-[var(--color-page-soft)] p-6 sm:flex-row sm:items-center">
          <div>
            <p className={`${display} text-[1.2rem]`}>Building something similar?</p>
            <p className="mt-1 text-[0.9rem] text-[var(--color-muted)]">Let&apos;s talk about your product.</p>
          </div>
          <a
