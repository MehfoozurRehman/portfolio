...
export function CursorHighlight() {
  ...
  useEffect(() => {
    const canUsePointerGlow = window.matchMedia("(pointer: fine)").matches && !window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    ...
    const updatePosition = () => {
      ...
      highlightRef.current?.style.setProperty("--cursor-x", `${pointerRef.current.x}px`);
      highlightRef.current?.style.setProperty("--cursor-y", `${pointerRef.current.y}px`);
    };
