...
const { profile, links, experience, education, stack, profileHighlights, services } = cvContent;
...
function text(value, x, y, o = {}) {
  const { size = 9.4, font = 'Helvetica', fill = c.text, width, lineGap = 1.6, align, characterSpacing = 0 } = o;
  doc.fillColor(fill).font(font).fontSize(size).text(value, x, y, { width, lineGap, align, characterSpacing });
}
...
function chip(label, x, y, maxW, dark) {
  ...
  doc
    ...
    .fillColor(dark ? c.darkChip : c.chip)
    .fill();
  doc
    .roundedRect(x, y, w, 15, 3.5)
    .lineWidth(0.7)
...
experience.forEach(([role, company, period, location, summary], i) => {
  ...
  const metaW = doc.widthOfString(metaText) + metaCS * Math.max(0, metaText.length - 1);
